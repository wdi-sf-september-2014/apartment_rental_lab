...
Building.prototype.addTenant = function(unit, tenant) {
  ...
  var unitDex = this.units.indexOf(unit);
  if ((tenant.references.length === 2) && (this.manager) && (unitDex > -1) && (unit.available())) {
    unit.tenant = tenant;
  }
};
...
Building.prototype.removeTenant = function(unit, tenant) {
  // remove tenant
  var unitDex = this.units.indexOf(unit);
  if ((unitDex > -1) && (unit.tenant === tenant) && (unit.available()=== false) && this.manager) {
    unit.tenant = null;
  }
};
